...
impl Engine {
    ...
    ///
    /// 说明：
    /// - 这里仅实现当前 rest 模块已落地的、语义稳定的部分（timeout / max_bytes）。
        ...
        S: Clone + Send + Sync + 'static,
    {
        ...
        if self.conf.middlewares.timeout && self.conf.timeout > 0 {
            ...
            router = router.layer(::axum::middleware::from_fn_with_state(
                super::middleware::timeout::axum_timeout::state(duration),
                super::middleware::timeout::axum_timeout::run,
            ));
        }

        // WithMaxBytes：默认使用 conf.MaxBytes
        if self.conf.middlewares.max_bytes {
            router = router.layer(super::middleware::max_bytes::axum_max_bytes::layer(
                self.conf.max_bytes,
            ));
        }
