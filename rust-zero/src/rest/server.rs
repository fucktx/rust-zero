...
///
/// - feature="axum": `app` 为 `axum::Router`
/// - feature="actix": `app` 为 `HttpServer::new(factory)` 所需的 factory
///
/// 注意：签名会随 feature 不同而变化；调用点建议写 `rest::server::start(engine, app).await?;`
/// 启动服务（对齐 go-zero 的 `server.Start()` 语义）。
///
/// 注意：为了避免 workspace `--all-features`（同时启用 axum+actix）下的重名冲突，
/// `start(...)` 只在 **恰好启用一个框架 feature** 时提供。
#[cfg(all(feature = "axum", not(feature = "actix")))]
pub async fn start(engine: crate::rest::engine::Engine, app: ::axum::Router) -> anyhow::Result<()> {
    crate::rest::native::axum::run(engine, app).await
}

#[cfg(all(feature = "actix", not(feature = "axum")))]
pub async fn start<F, I, S, B>(
...
{
    crate::rest::native::actix::run(engine, factory).await
}
...
#[cfg(feature = "axum")]
impl Server<()> {
    pub fn new<S>(conf: crate::rest::RestConf) -> anyhow::Result<Server<S>>
    where
        S: Clone + Send + Sync + 'static,
    {
        conf.validate().map_err(anyhow::Error::msg)?;
        Ok(Server {
            engine: crate::rest::engine::Engine::new(conf),
            router: ::axum::Router::<S>::new(),
        })
    }

    pub fn must_new<S>(conf: crate::rest::RestConf) -> Server<S>
    where
        S: Clone + Send + Sync + 'static,
    {
    ...
    S: Clone + Send + Sync + 'static,
{
    ...
    pub fn conf(&self) -> &crate::rest::RestConf {
        &self.engine.conf
    }
}
...
impl Server<()> {
    pub async fn start(self) -> anyhow::Result<()> {
        ...
        crate::rest::native::axum::run(engine, router).await
    }
}

/// axum 适配：`cargo add rust-zero --features axum`
#[cfg(feature = "axum")]
pub mod axum {
    use crate::rest::engine::Engine;
    use anyhow::Context;
...
/// actix-web 适配：`cargo add rust-zero --features actix`
#[cfg(feature = "actix")]
pub mod actix {
    use crate::rest::engine::Engine;
...
mod tests {
    ...
    fn axum_run_signature_should_compile() {
        use crate::rest::{config::RestConf, engine::Engine};
        let engine = Engine::new(RestConf::default());
        let app = ::axum::Router::<()>::new();
        let _fut = crate::rest::server::axum::run(engine, app);
        // 不 await：仅做类型/签名编译验证
    }
    ...
    fn actix_run_signature_should_compile() {
        use crate::rest::{config::RestConf, engine::Engine};
        let engine = Engine::new(RestConf::default());
        let _fut = crate::rest::server::actix::run(engine, actix_web::App::new);
        // 不 await：仅做类型/签名编译验证
    }
}
