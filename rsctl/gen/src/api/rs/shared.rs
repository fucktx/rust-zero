...
fn find_annotation<'a>(anns: &'a [spec::api::Annotation], name: &str) -> Option<&'a spec::api::Annotation> {
    anns.iter().find(|a| a.name == name)
}

fn route_handler_name(route: &spec::api::Route) -> Option<String> {
    let Some(a) = find_annotation(&route.annotations, "handler") else { return None };
    match &a.args {
...
fn service_group_name(service: &spec::api::Service) -> Option<String> {
    let Some(srv) = find_annotation(&service.annotations, "server") else { return None };
...
fn parse_style(style: &str) -> Result<Style> {
    match style {
        ...
        other => Err(anyhow!("unsupported style: {other} (expected rust_zero|rustZero|RustZero)")),
    }
}
...
"#
    ...
    files.push(Artifact {
        ...
        content: tpl_render(&etc_tpl, &base_ctx)
    ...
    files.push(Artifact {
        ...
        content: tpl_render(&main_tpl, &base_ctx)
            .context("render main.tpl")?,
    });

    files.push(Artifact {
        rel_path: "config.rs".into(),
        content: tpl_render(&config_tpl, &base_ctx)
            .context("render config.tpl")?,
    });
    files.push(Artifact {
        rel_path: "svc.rs".into(),
        content: tpl_render(&svc_tpl, &base_ctx)
            .context("render svc.tpl")?,
    });
            ...
            content: {
                ...
                for h in hs {
                    ...
                    let ctx = base_ctx
                        ...
                        .set_str("Doc", doc.as_ref().map(|d| format!("/// {}\n", d)).unwrap_or_default())
