...
fn generate_by_framework(
    ...
    opts: &Options,
) -> anyhow::Result<crate::code::artifact::Artifacts> {
...
pub mod shared {
    use crate::code::artifact::{Artifact, Artifacts};
    ...
    fn find_monorepo_root(out_dir: &Path) -> Option<PathBuf> {
        ...
        loop {
            // rust-zero 约定：仓库根下存在 core/rest/rsctl 三个 crate。
            if dir.join("core").join("Cargo.toml").is_file()
                && dir.join("rest").join("Cargo.toml").is_file()
                && dir.join("rsctl").join("Cargo.toml").is_file()
            {
                return Some(dir);
            }
            if !dir.pop() {
    ...
    fn find_kv<'a>(ann: &'a crate::spec::api::Annotation, key: &str) -> Option<&'a str> {
        match &ann.args {
            crate::spec::api::AnnotationArgs::Map(kvs) => {
    ...
    fn route_handler_name(route: &crate::spec::api::Route) -> Option<String> {
        let a = find_annotation(&route.annotations, "handler")?;
        match &a.args {
            crate::spec::api::AnnotationArgs::Str(s) => Some(s.clone()),
            _ => None,
        }
    }

    fn service_group_name(service: &crate::spec::api::Service) -> Option<String> {
        let srv = find_annotation(&service.annotations, "server")?;
        find_kv(srv, "group").map(|s| s.to_string())
    }

    fn effective_group(service: &crate::spec::api::Service) -> String {
        // go-zero 习惯：没写 group 时，默认用 service 名作为 group。
        service_group_name(service).unwrap_or_else(|| service.name.clone())
    }

    fn service_prefix(service: &crate::spec::api::Service) -> Option<String> {
        let srv = find_annotation(&service.annotations, "server")?;
        find_kv(srv, "prefix").map(|s| s.to_string())
    }

    fn service_middleware(service: &crate::spec::api::Service) -> Option<String> {
        let srv = find_annotation(&service.annotations, "server")?;
        find_kv(srv, "middleware").map(|s| s.to_string())
    }

    fn service_jwt(service: &crate::spec::api::Service) -> Option<String> {
        let srv = find_annotation(&service.annotations, "server")?;
                        ...
                        'outer: for service in &spec.services {
                            ...
                            for r in &service.routes {
                                ...
                                if let Some(a) = r.annotations.iter().find(|a| a.name == "doc")
                                    && let crate::spec::api::AnnotationArgs::Str(s) = &a.args
                                {
                                    doc = Some(s.clone());
                                }
                                break 'outer;
