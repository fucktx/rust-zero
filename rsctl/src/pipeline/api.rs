...
pub mod rs {
    ...
    pub(crate) fn config(mut opts: Options, web: &str) -> Result<Config> {
        ...
        let template_root = crate::utils::template::resolve_template_root(opts.remote.as_deref())?;
    ...
    pub(crate) fn pipeline(cfg: &Config, web: &str) -> Result<()> {
        // 1) parse
        let ast = crate::parse::api::parse_file(&cfg.api_file).context("parse api file")?;

        // 2) semantic -> spec (stable IR)
        let spec = crate::semantic::api::to_spec(&ast).context("semantic to spec")?;
        ...
        let artifacts = match web {
            "axum" => crate::code::api::rs::axum::generate(
                &spec,
                &crate::code::api::rs::axum::Options {
                    service_name,
                    merge: cfg.merge,
            ...
            "actix" => crate::code::api::rs::actix::generate(
                &spec,
                &crate::code::api::rs::actix::Options {
                    service_name,
                    merge: cfg.merge,
                    style: style.to_string(),
                    out_dir: cfg.out_dir.clone(),
    ...
    fn write_artifacts(cfg: &Config, artifacts: &crate::code::artifact::Artifacts) -> Result<()> {
