...
mod tests {
    ...
    static FIXTURES: Dir<'_> = include_dir!("$CARGO_MANIFEST_DIR/templates");
    ...
    fn test_extract_dir_writes_files() {
        ...
        // 选取仓库内置模板中的确定性文件做断言
        let api_txt = tmp.join("api").join("rs").join("api.txt");
        let main_tpl = tmp.join("api").join("rs").join("main.tpl");
        assert!(api_txt.is_file());
        assert!(main_tpl.is_file());
        assert!(!std::fs::read_to_string(api_txt).unwrap().trim().is_empty());
    }
}
