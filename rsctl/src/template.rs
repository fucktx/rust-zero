...
fn init(overwrite: bool) -> Result<()> {
    let (rsctl_home, version_dir) = crate::utils::path::rsctl_version_dir(version::VERSION)?;
    ...
    crate::utils::template::install::extract_dir(&EMBED_TEMPLATES, &version_dir)?;
...
fn clean() -> Result<()> {
    let (_, version_dir) = crate::utils::path::rsctl_version_dir(version::VERSION)?;
