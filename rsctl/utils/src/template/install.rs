...
mod tests {
    ...
    static FIXTURES: Dir<'_> = include_dir!("$CARGO_MANIFEST_DIR/tests/fixtures/mini_templates");
    ...
    fn test_extract_dir_writes_files() {
        ...
        let nested = tmp.join("nested").join("b.txt");
        assert!(a.is_file());
        assert!(nested.is_file());
        assert_eq!(std::fs::read_to_string(a).unwrap().trim(), "A");
        assert_eq!(std::fs::read_to_string(nested).unwrap().trim(), "B");
    }
}
